SELECT
  ...
  coll_event_remarks,
  habitat,
  DECODE(verbatim_coordinates,NULL,'',('Verbatim coordinates:' || verbatim_coordinates)) coordinates,
  DECODE(identifiedby,'unknown','',('Det. ' || identifiedby)) formatted_identifiedby,
  DECODE(collection_id,'124','BOTANY COLLECTION','144','TEACHING COLLECTION',NULL) collection,
  DECODE(CONCATSINGLEOTHERID(collection_object_id,'collector number'),NULL,collectors,(collectors || '; ' || CONCATSINGLEOTHERID(flat.collection_object_id,'collector number'))) collectors_numbers,
